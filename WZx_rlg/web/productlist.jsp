...
<div class="main">
    ...
    <div class="frameMain">
        <div class="title" id="frameMainTitle">
            <span><i class="iconfont icon-xianshiqi"></i>商品列表</span>
        </div>
        <%--//内容显示区域--%>
...
<script>
    $(function () {
            $.get(
                "/manage/product/plist.do",
                function (dt) {
                    if (dt.status !== 0){
                        alert(dt.mag);
                    }else{
                        for (var i = 0;i <dt.data.length;i++){
                            var s = "<tr><td>"+dt.data[i].id+"</td><td>"+dt.data[i].pname+"</td>" +
                                "<td>"+dt.data[i].parentId+"</td><td>"+dt.data[i].status+"</td></tr>";
                            $("#tab").append(s);
                        }
                    }
                },
